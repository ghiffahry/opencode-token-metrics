...
import { $, nf, esc, clamp, formatTokens, formatLatency, formatRelative } from "../core/utils.js";
...
export function renderModelTable(view) {
  ...
  $("#modelBody").innerHTML = list.map(function (m) {
    ...
    return (
      ...
      '<td class="numeric">' + nf.format(m.requests) + "</td>" +
      '<td class="numeric">' + formatTokens(m.input) + "</td>" +
      '<td class="numeric">' + formatTokens(m.output) + "</td>" +
      '<td class="numeric"><strong>' + formatTokens(m.input + m.output) + "</strong></td>" +
      '<td class="numeric">' + (m.errors ? nf.format(m.errors) : "0") + "</td>" +
      '<td class="numeric">' + rate.toFixed(2) + "%</td>" +
      '<td class="numeric">' + formatLatency(m.latency) + "</td>" +
...
export function renderSessionTable() {
  ...
  var rows = list.map(function (s) {
    ...
    return (
      "<tr>" +
      '<td><span class="cell-primary mono">' + s.id + "</span><br><span class=\"cell-secondary\">" + esc(s.title || s.user) + "</span></td>" +
...
export function getFilteredRequests() {
  ...
  if (state.requestSearch) {
    ...
    list = list.filter(function (r) { return r.id.toLowerCase().indexOf(q) !== -1; });
  }
  return list;
}
...
export function renderRateLimit(key, data, fmt) {
  ...
  var pct = known ? clamp((data.used / data.limit) * 100, 0, 100) : 0;
  var cls = pct >= 90 ? "is-danger" : pct >= 80 ? "is-warning" : "";
  $("#" + key + "Used").textContent = fmt(data.used);
  $("#" + key + "Limit").textContent = known ? nf.format(data.limit) : "?";
  $("#" + key + "Bar").style.width = pct.toFixed(1) + "%";
  $("#" + key + "Bar").className = "progress__bar " + cls;
  $("#" + key + "Pct").textContent = known ? pct.toFixed(1) + "%" : "unknown limit";
  var badge = $("#" + key + "Badge");
  if (!known) { badge.className = "status-badge"; badge.textContent = "Unknown"; }
  else if (pct >= 90) { badge.className = "status-badge status-badge--danger"; badge.textContent = "Critical"; }
  else if (pct >= 80) { badge.className = "status-badge status-badge--warning"; badge.textContent = "Warning"; }
  else { badge.className = "status-badge status-badge--success"; badge.textContent = "OK"; }
  var rem = $("#" + key + "Remaining");
  if (rem) rem.textContent = known ? nf.format(Math.max(0, data.limit - data.used)) : "unknown";
}
